...
log('\n=== EN LIGNE : chaque lecture du serveur laisse une copie sur le téléphone (par employé) ===');
{
  ...
  eq('les copies : arrêts, routes, tours, véhicules et équipages, problèmes, employés, équipage précédent, équipe de fin (étape 17), véhicules actifs — chacune sous « cache:<employé>:<nom> »', Object.keys(mem).sort(), ['stops', 'routes', 'tours', 'vehicules', 'problemes', 'employes', 'equipagePrecedent', 'equipeDeFin', 'equipesActives'].sort().map((n) => 'cache:u-luc:' + n));
...
log('\n=== LE CODE : plus aucune dépendance à Internet au démarrage ===');
{
  ...
  vrai('la boussole se range sous les QUATRE boutons ronds (satellite, plan, position, tracé : 44 px, même style, 8 px d\'écart) et l\'aiguille en couleur montre la rotation active', /\.leaflet-top\.leaflet-right\{margin-top:calc\(296px \+ var\(--sa-top\)\);\}/.test(css) && /--lrc-control-size:44px/.test(css) && /\.leaflet-control-rotate:not\(\.leaflet-control-rotate--inactive\)\{border-color:var\(--accent\);\}/.test(css));
